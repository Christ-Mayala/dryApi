...
| `monitoring/`                      | Prometheus + Grafana (dashboard 13 panels + alertes) — `docker-compose --profile monitoring up -d` les lance et les configure automatiquement (provisioning). Voir `monitoring/grafana/readme.md`.                                |
...
| `LICENSE`                          | Licence propriétaire (Community gratuit / Pro-Enterprise sous contrat commercial) — pas open source.                                                                                                                              |
| `CONTRIBUTING.md`                  | Workflow de contribution (setup, tests, conventions).                                                                                                                                                                             |
...
| `http/`                         | `response.js` (`sendResponse` — le format JSON standard de toute l'API), `httpError.js` (`httpError(message, statusCode)` — à utiliser pour toute erreur métier attendue, sinon `errorHandler.js` la traite comme un bug 500 et alerte dessus), `cookies.js` (options des cookies JWT), `ip.js` (IP client, avec sel de hachage pour l'anonymisation). |
