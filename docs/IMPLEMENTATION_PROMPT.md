...
| 8. Monitoring dashboard    | ✅ Fait, 1 gap comblé | Le système d'alerte (`alert.service.js`) déclenchait une alerte "CRITIQUE" sur un simple mauvais mot de passe : le contrôleur de login lançait une `Error` brute sans code HTTP, donc classée comme bug 500 au lieu d'un 4xx attendu. Corrigé avec `dry/utils/http/httpError.js`, appliqué à tout le contrôleur d'auth kernel + Trivida — testé en conditions réelles (401 correct, zéro alerte). Dashboard Grafana enrichi (+4 panels : requêtes par app, échecs de connexion informatifs, connexions actives, cache hit ratio) et les 4 seuils d'alerte, jusqu'ici juste écrits en prose dans le readme, sont maintenant de vraies règles importables (`monitoring/grafana/alerting-rules.yml`). La page de statut publique (`status.dryapi.onrender.com`) reste un service externe (type Statuspage.io) — hors du périmètre du code. |
| 9. Tarification/Commercial | ✅ Fait               | `landing/pricing.html`, `dry/modules/billing/` (Stripe), `dry/modules/licensing/`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 10. Finitions              | ✅ Fait               | README à jour, CHANGELOG, `.env.example`, scripts npm corrigés (`logs`, `test:coverage`→`coverage`), [CONTRIBUTING.md](../CONTRIBUTING.md) ajouté, [LICENSE](../LICENSE) propriétaire créée (Community gratuit / Pro-Enterprise sous contrat).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |

**Ce qui reste vraiment ouvert :** la page de statut publique externe (phase 8) et l'import effectif des règles d'alerte dans une vraie instance Grafana (opérationnel, pas du code) — tout le reste de ce document est soit fait, soit adapté différemment du plan d'origine avec le même résultat.
