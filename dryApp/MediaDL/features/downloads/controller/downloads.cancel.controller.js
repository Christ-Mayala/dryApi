...
module.exports = asyncHandler(async (req, res) => {
  ...
  if (!['pending', 'running'].includes(item.jobStatus)) {
    return sendResponse(res, item, 'Telechargement deja termine', true);
  }
  ...
  const updated = await Download.findByIdAndUpdate(
    item._id,
    { jobStatus: 'cancelled', error: 'Annule par utilisateur', finishedAt: new Date() },
    { new: true }
  );

  return sendResponse(res, updated, 'Telechargement annule', true);
});
