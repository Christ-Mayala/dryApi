...
const ADMIN_EMAIL =
  process.env.E2E_SCIM_ADMIN_EMAIL || process.env.SEED_ADMIN_EMAIL || 'admin@admin.com';
const ADMIN_PASSWORD =
  process.env.E2E_SCIM_ADMIN_PASSWORD || process.env.SEED_ADMIN_PASSWORD || 'Admin@2026';
...
const pickProperty = async (requesterId) => {
  ...
  const candidate = properties.find(
    (p) => pickPropertyOwnerId(p) && pickPropertyOwnerId(p) !== requesterId
  );
  if (!candidate) {
    throw new Error(
      'No property available for requester. Set E2E_SCIM_PROPERTY_ID to a property not owned by requester.'
    );
  }
...
const main = async () => {
  ...
  const visit1 = buildScimLocalDate({
    offsetMinutes: SCIM_OFFSET,
    dayOffset: 1,
    hour: 11,
    minute: 0,
  });
  const requesterPhone = String(
    requester?.user?.telephone || requester?.telephone || REQUESTER_PHONE || ''
  ).trim();
  ...
  const visit2 = buildScimLocalDate({
    offsetMinutes: SCIM_OFFSET,
    dayOffset: 2,
    hour: 14,
    minute: 0,
